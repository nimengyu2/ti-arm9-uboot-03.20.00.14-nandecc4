...
#else
	const int region = CONFIG_SYS_NAND_CS;
...
static void nand_flash_init(void)
{
	...
	emif_regs->AWCCR |= 0x000000FF;
	emif_regs->AB1CR = acfg2;	/* 0x08244128 */;
	emif_regs->AB2CR = acfg2;
	emif_regs->AB3CR = acfg2;
	emif_regs->AB4CR = acfg2;
	emif_regs->NANDFCR = 0x00000204; // 202
}

int davinci_nand_init(struct nand_chip *nand)
{
...
/*
 * davinci_nand_switch_ecc - switch the ECC operation b/w h/w ecc and s/w ecc.
 * The default is to come up on s/w ecc
 *
 * @hardware - 1 -switch to h/w ecc, 0 - s/w ecc
 *
 */
void davinci_nand_switch_ecc(int32_t hardware)
{
